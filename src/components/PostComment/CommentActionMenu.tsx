  ...
  return (
    <div>
     <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button as="div" className="flex items-center dark
         bg-transparent text-gray-100 hover:text-gray-600 cursor-pointer
          focus:outline-none focus:ring-2
           focus:ring-indigo-500 focus:ring-offset-2
            focus:ring-offset-gray-100">
            <IoEllipsisHorizontal className="h-10 w-10 p-2" 
            name="feedMenu" aria-hidden="true" />
        </Menu.Button>
      </div>
              ...
              {({ active }) => (
                ...
                >
                  <div className="flex text-slate-100 pointer-events-none">
                    ...
                    <div className="pl-2 text-sl">{t('report')}</div>
                  </div>
                </a>
              )}
            </Menu.Item>
    ...
    <ReportModal  
    //@ts-expect-error
    name="feedMenu"  setReportState={setReportType} send={Report} open={open} setOpen={setOpen} />
    </div>
  );
}
