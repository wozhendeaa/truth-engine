...
export default async function handler(req:  NextApiRequest, res: NextApiResponse) {
   ...
   try {
        //检查用户名和邮件是否存在
        const isUserNameTaken = await caller.user.isUsernameTaken({username: userFormData.username});
        const isEmailTaken = await caller.user.isEmailTaken({email: userFormData.email});
        ...
        await caller.user.registerInMyDatabase(userFormData);
